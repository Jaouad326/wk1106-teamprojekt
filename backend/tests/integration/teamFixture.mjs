// Prüft alle Module aus demselben aktuellen Checkout mit einer isolierten Testdatenbank.
...
export async function teamFixture({ onMail = () => {} } = {}) {
  ...
  try {
    await authMigration(db); await groupMigration(db); await taskMigration(db); await commentMigration(db);
